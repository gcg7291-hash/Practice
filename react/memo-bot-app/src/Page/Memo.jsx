...
// ⭐️ Vercel Base URL 환경 변수 사용
const VERCEL_BASE_URL = import.meta.env.VITE_VERCEL_BASE_URL || ''; 
...
    // ⭐️ 404 에러 해결을 위해 VERCEL_BASE_URL을 사용하여 전체 URL을 구성합니다.
    const apiUrl = `${VERCEL_BASE_URL}/api/ai/generate-memo`; 

    try {
      const response = await axios.post(apiUrl, { // 수정된 apiUrl 사용
...
        createdAt: parsedData.createdAt || new Date().toISOString().slice(0, 10),
...
      // 서버리스 함수 응답(404, 500 등)에서 에러 메시지를 추출하여 사용자에게 표시
      const errorMessage = error.response?.data?.error 
                         || "AI 메모 생성에 실패했습니다 (네트워크 오류, Vercel 배포 확인 필요).";
                         
      setMessages((prev) => [
        ...prev,
