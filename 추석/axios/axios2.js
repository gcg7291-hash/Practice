...
//   const res = await axios.get(`${BASE_URL}/carts/1`)
//   const data = res["data"]
//   console.log(data)
// }
// getSingleCart()
...
async function getSingleCartFirstProduct() {
  const res = await axios.get(`${BASE_URL}/carts/1`);
  const data = res["data"];
  const firstId = data.products[0];
  const firstPId = firstId["id"];
  console.log(firstPId);
  const resI = await axios.get(`${BASE_URL}/products/${firstPId}`);
  const dataI = resI["data"];
  console.log(dataI);
}
