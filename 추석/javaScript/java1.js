...
// 구조와 컨텐츠를 만드는 기본 단위, <></>태그의 시작 종료 <h1~h6> 숫자가 커질수록 텍스트 크기가 작아짐 <p>단락 <a>하이퍼링크 herf 속성으로 링크 대상 지정 <br>줄바꿈
// <img> <audio> <video> src 경로 지정 , 속성은 추가적인 정보와 기능제공 class 그룹화 속성, style 스타일 적용속성 src 경로지정 herf 링크이동
// 문제 [개념]
// HTML 속성 중 class 속성의 역할에 대해 작성한다
// class 그룹속성 하나의 클래스를 지정하거나 여러개의 요소를 지정가능
// 문제 [개념]
// JavaScript 원시 자료형 중 기본 자료형 3개에 대해 작성한다
// 문자열, 숫자형, 불리언
// 문제 [개념]
// 변수 선언 키워드 let, const의 차이점을 작성한다
//let은 변수를 지정을 해도 바꿀수 잇음 const는 변수를 지정하면 바꿀수없음
// 문제 [실습]
...
// a>b a가 크다, a<b b가 크다, a>=b a가b랑 같거나 크다, a<=b a가b랑 같거나 작다, a===b 엄격하게 비교 같다, a !== b 엄격하게 틀리다 (값과 자료형 모두 비교)
// 문제 [실습]
...
let isLogin = false;
const result2 = isLogin === true ? "로그인상태입니다" : "로그아웃 상태입니다";
console.log(result2);
/* 출력 결과
로그아웃 상태입니다
*/
...
// key value 이다 , [] .으로 접근 공백이나 특수문자 []사용 , . [] 수정할 속성에 접근하여 추가, delete 객체.속성명
// 문제 [실습]
...
function processScore(student) {
  const score20 = student["score"];
  const scores = score20.reduce((a, b) => {
    return a + b;
  });
  const numb = scores["length"];
  const averageScore = scores / numb;
...
// map 콜백 함수의 반환값을 원소로 모아 새로운 배열 생성 , filter 콜백함수의 반환값이 true인 원소만 모아 새로운 배열 생성
// 문제 [개념]
// 배열의 고차 메서드 map()과 filter()의 차이점을 작성한다
// map배열의 요소를 변형하거나 변환하는 것.항상 원본 배열과 길이가 같은 새로운 배열.	filter 배열의 요소를 걸러내어(선택) 원하는 조건에 맞는 요소만 남기는 것.원본 배열보다 길이가 같거나 짧은 새로운 배열.
// 문제 [실습]
// 변수 numbers3를 선언하고, 배열 데이터 [1, 2, 3, 4, 5]를 할당한다
// numbers3의 각 요소를 제곱한 새로운 배열을 생성하고, 변수 newNumbers에 할당한다.
// 변수 newNumbers를 출력한다
let number3 = [1, 2, 3, 4, 5];
let newNumbers = number3.map((number) => {
...
let taxIncludedPrices = prices.map((tax) => {
  return tax * 1.1;
});
...
let newNumbers2 = numbers4.filter((number) => {
  if (number % 2 === 0) {
    return number;
  }
});
...
let highPriorityTodos = originTodos.filter((p) => {
  if (p["priority"] === 0) {
    return p;
  }
});
...
let mediumPriorityTodos = originTodos.filter((p) => {
  if (p["priority"] === 1) {
    return p;
  }
});
...
let incompleteTodos = originTodos.filter((p) => {
