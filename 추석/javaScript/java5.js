...
function findUserByName(a, b) {
  return a.find((user) => user["name"] === b);
}
try {
  const users = [
...
function calculateTotalPrice(a) {
  return a.reduce((b, i) => {
    const allPrice = i["price"] * i["quantity"];
    return b + allPrice;
  }, 0);
}
try {
...
function sortStudentsByScore(student) {
...
function areAllNumbersGreaterThan(a, b) {
  return a.every((value) => value > b);
}
try {
...
function countStringOccurrences(a) {
  return a.reduce((b, c) => {
    if (b[c]) {
      b[c] += 1;
    } else {
...
function isPalindrome(a) {
  const revers = a.split("").reverse().join("");
  return a === revers;
}
try {
...
function getProductsByCategory(a, b) {
