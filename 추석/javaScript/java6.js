...
function findTopStudent(a, b) {
  const topScore = a.reduce((maxName, score) => {
    const maxScore = maxName.scores[b];
    const newScore = score.scores[b];
...
function groupByCategory(a) {
  return a.reduce((b, c) => {
    const newCategory = c["category"];
    if (b[newCategory]) {
      b[newCategory].push(c);
    } else {
      b[newCategory] = [c];
    }
    return b;
